...
### **Katalog workspace:**
```bash
sudo mkdir -p /root/mordzix-ai
sudo chown -R $USER:$USER /root/mordzix-ai
cd /root/mordzix-ai
```
...
scp -r C:\Users\48501\Desktop\mrd\* root@51.83.131.142:/root/mordzix-ai/
```
...
- User: `root`
- Port: `22`
- Upload całego folderu `mrd` do `/root/mordzix-ai/`

### **Struktura katalogów:**
```bash
mkdir -p /root/mordzix-ai/logs
mkdir -p /root/mordzix-ai/out/uploads
mkdir -p /root/mordzix-ai/data
chmod -R 755 /root/mordzix-ai
```
...
```bash
cat > /root/mordzix-ai/.env << 'EOF'
# Mordzix AI - Production Configuration
AUTH_TOKEN=ssjjMijaja6969
WORKSPACE=/root/mordzix-ai
MEM_DB=/root/mordzix-ai/mem.db
UPLOAD_DIR=/root/mordzix-ai/out/uploads
...
server {
    ...
    # Logi
    access_log /root/mordzix-ai/logs/nginx_access.log;
    error_log /root/mordzix-ai/logs/nginx_error.log;
...
[program:mordzix]
directory=/root/mordzix-ai
command=/root/mordzix-ai/.venv/bin/gunicorn -w 4 -k uvicorn.workers.UvicornWorker -b 127.0.0.1:8080 --timeout 120 --access-logfile /root/mordzix-ai/logs/gunicorn_access.log --error-logfile /root/mordzix-ai/logs/gunicorn_error.log app:app
autostart=true
autorestart=true
stderr_logfile=/root/mordzix-ai/logs/supervisor_error.log
stdout_logfile=/root/mordzix-ai/logs/supervisor_out.log
user=root
environment=PATH="/root/mordzix-ai/.venv/bin"
EOF
```
...
# Supervisor
tail -f /root/mordzix-ai/logs/supervisor_error.log
```
...
- [ ] Pliki projektu w `/root/mordzix-ai/`
