...
async def test_search(
    ...
    """
    try:
        results = await ltm_search_hybrid(query=q, limit=limit, user_id=user_id)
        return {
            "ok": True,
            "query": q,
        ...
        return {"ok": True, "query": q, "total": 0, "results": [], "error": str(e)[:50]}
...
async def search_stats(user_id: str = "default", _=Depends(_auth)):
    ...
    try:
        ...
        return {
            ...
            "note": f"Stats unavailable: {str(e)[:50]}"
        }
...
<body>
    <div class="container">
        ...
        <div class="search-box">
            <input type="password" id="authTokenInput" placeholder="Auth Token" style="width: 150px;">
    ...
    <script>
        // Token is entered by user in the input field, not hardcoded
        function getAuthToken() {
            return document.getElementById('authTokenInput')?.value || '';
        }
        ...
        async function loadStats() {
            const AUTH_TOKEN = getAuthToken();
            if (!AUTH_TOKEN) return; // Skip if no token entered yet
            try {
